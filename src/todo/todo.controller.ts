import { Body, Controller, Delete, Get, Logger, NotFoundException, Param, Post, Put, Query, Req } from "@nestjs/common";
...
import { Payload } from "../auth/model/payload.model";
...
export class TodoController {
  ...
  @ApiQuery({
    name: "q",
    description: "Search keyword. Search todo by Title and description",
    required: false,
    type: String
  })
  @ApiQuery({
    name: "limit", description: "The number to limit the response",
    required: false, type: Number
  })
  public async getAllTodos(@Query("q") search: string = null,
                           @Query("limit") limit: number = null,
                           @Req() request: Request) {
    //return 'get all todos';
    let user: Payload = request["user"];
    let todos = await this.todoService.searchTodo(search, limit, user.uuid);
  ...
  @Get("/:id")
  public async getTodoById(@Param("id") uuid: string, @Req() request: Request) {
    let user: Payload = request["user"];
    let todo = await this.todoService.getTodoByUuid(uuid, user.uuid);
    if (!todo) {
  ...
  @Post()
  public async create(@Body() createTodo: CreateTodoDto,@Req() request: Request) {
    let user: Payload = request["user"];
    let todo = await this.todoService.createTodo(createTodo, user.uuid);
  ...
  public async update(@Body() updateTodo: UpdateTodoDto,
